"""Broad US-stock discovery for Strong Gainers, kept at project root for Streamlit stability."""
...
def _yahoo_custom_gainers(min_price: float, max_price: float, threshold: float) -> pd.DataFrame:
    try:
        ...
        payload = yf.screen(query, size=250, sortField="percentchange", sortAsc=False)
    ...
    for q in quotes:
        ...
        if symbol and min_price <= price <= max_price and pct >= threshold:
            rows.append({"symbol":symbol,"market_price":round(price,4),"market_change_pct":round(pct,2),"market_volume":volume,"exchange":str(q.get("fullExchangeName") or q.get("exchange") or ""),"source":"Market discovery"})
...
def _nasdaq_gainers(min_price: float, max_price: float, threshold: float) -> pd.DataFrame:
    ...
    for exchange in ("nasdaq", "nyse", "amex"):
        ...
        for row in rows:
            ...
            if symbol and min_price <= price <= max_price and pct >= threshold:
                rows_out.append({"symbol":symbol,"market_price":round(price,4),"market_change_pct":round(pct,2),"market_volume":volume,"exchange":exchange.upper(),"source":"Market discovery"})
    return pd.DataFrame(rows_out)


@st.cache_data(ttl=180, show_spinner=False)
def discover_market_gainers(min_price: float = 0.40, max_price: float = 50.0, threshold: float = 40.0) -> pd.DataFrame:
    frames = [_yahoo_custom_gainers(min_price,max_price,threshold), _nasdaq_gainers(min_price,max_price,threshold)]
    ...
    combined = combined.sort_values(["market_change_pct","market_volume"], ascending=False).drop_duplicates("symbol", keep="first")
    ...
    for exchange in ("nasdaq","nyse","amex"):
        try:
            for row in _fetch_exchange_rows(exchange):
                ...
                if symbol:symbols.add(str(symbol))
...
def get_universe(limit: int | None = None) -> tuple[str, ...]:
    symbols=load_us_universe(); return symbols[:limit] if limit and limit>0 else symbols


def universe_status() -> dict[str, object]:
    symbols=load_us_universe(); return {"count":len(symbols),"source":"US market universe","cached_hours":6}
