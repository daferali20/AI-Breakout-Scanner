...
def render_market_data():
    """دالة توافقية للاستدعاء باسم الصفحة مباشر"""
    render()
...
def display_stock_table(df):
    ...
    display_df = df.copy()
    display_df['price'] = display_df['price'].apply(lambda x: f"${x:.2f}" if isinstance(x, (int, float)) and x > 0 else "N/A")
    display_df['change'] = display_df['change'].apply(lambda x: f"${x:.2f}" if isinstance(x, (int, float)) and x != 0 else "$0.00")
    display_df['change_percent'] = display_df['change_percent'].apply(lambda x: f"{x:.2f}%" if isinstance(x, (int, float)) else "0.00%")
...
def display_stock_analysis(symbol):
    with st.spinner(f"جاري تحليل {symbol}..."):
        ...
        if len(df) > 20:
            ...
            fig.add_trace(go.Scatter(x=df.index, y=ma20, line=dict(color='#FFD700', width=1.5), name="MA20"))
        
        fig.update_layout(template="plotly_dark", height=400, margin=dict(l=20, r=20, t=40, b=20), xaxis_rangeslider_visible=False)
    ...
    with st.spinner("جاري جلب بيانات المؤشرات..."):
        ...
        for symbol, name in US_INDICES.items():
            try:
                ...
                indices_data[symbol] = {
                    'symbol': symbol, 'name': name, 'price': current, 'change': change, 'change_percent': pct
                }
            except:
                continue
        ...
        for idx, (symbol, data) in enumerate(indices_data.items()):
            with cols[idx % 4]:
                ...
                st.markdown(f"""
                <div style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; padding: 15px; margin: 5px 0; text-align: center;">
                    ...
                    <div style="color:{color}; font-weight:700;">{data['change']:+.2f} ({data['change_percent']:+.2f}%)</div>
                </div>
...
def render_etfs():
    ...
    selected_group = st.selectbox("تصفية حسب النوع:", list(etf_groups.keys()), key="etf_group")
    ...
    with st.spinner("جاري جلب بيانات الصناديق..."):
        ...
        st.dataframe(
            ...
            column_config={
                "symbol": "الرمز", "name": "الاسم", "price": "السعر", "change_percent": "التغير %", "volume": "حجم التداول"
            },
...
def render_search():
    ...
    search_term = st.text_input("ابحث عن رمز أو اسم شركة:", placeholder="مثال: AAPL, Microsoft...", key="search_input")
    ...
    if search_term:
        ...
        if results:
            ...
            selected = st.selectbox("اختر رمزاً للتحليل:", [r['symbol'] for r in results], key="search_result_select")
            if selected:
