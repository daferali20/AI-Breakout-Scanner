...
class TimelineBuilder:
    ...
    def _get_previous_phases(self, current_phase: MarketPhase) -> List[MarketPhase]:
        """الحصول على المراحل السابقة المحتملة"""
        # تسلسل المراحل النموذجي
        sequence = [
            MarketPhase.DECLINE,
    ...
    def _get_phase_confidence(self, phase: MarketPhase, data: Dict[str, Any]) -> float:
        """الحصول على ثقة المرحلة"""
        # تحسين هذه القيم بناءً على البيانات المتاحة
        base_confidence = {
            MarketPhase.ACCUMULATION: 0.85,
            MarketPhase.COMPRESSION: 0.90,
            MarketPhase.MOMENTUM: 0.80,
            MarketPhase.BREAKOUT_READY: 0.85,
            MarketPhase.BREAKOUT: 0.75,
            MarketPhase.TREND_CONTINUATION: 0.70,
            MarketPhase.DISTRIBUTION: 0.60,
            MarketPhase.DECLINE: 0.50,
        }.get(phase, 0.70)
