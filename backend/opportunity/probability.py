...
class ProbabilityCalculator:
    ...
    def calculate_success_probability(self, phase: MarketPhase,
        ...
        # تحديد الاحتمال النهائي
        probability = min(98, max(10, base_prob + adjustments))
        
        return probability
    
    def _get_base_probability(self, phase: MarketPhase) -> float:
        """الحصول على الاحتمال الأساسي حسب المرحلة"""
        base_probs = {
            MarketPhase.ACCUMULATION: 65,
            MarketPhase.COMPRESSION: 75,
            MarketPhase.MOMENTUM: 70,
            MarketPhase.BREAKOUT_READY: 80,
            MarketPhase.BREAKOUT: 60,
            MarketPhase.TREND_CONTINUATION: 55,
            MarketPhase.DISTRIBUTION: 30,
