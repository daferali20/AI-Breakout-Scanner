...
_LOCK = RLock()
_STORE: dict[str, Any] = {
    "scan_results": pd.DataFrame(), "scan_results_all": pd.DataFrame(), "ranked_results": pd.DataFrame(),
...
def _build_ranked(frame: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    if frame.empty:
        return pd.DataFrame(), {"regime": "NEUTRAL", "score": 50.0, "label": "🟡 محايد"}
    try:
    ...
    try:
        ...
        pass
    try:
        from backend.market_intelligence import enrich_market_intelligence
        ranked, regime = enrich_market_intelligence(ranked)
    except Exception:
        regime = {"regime": "NEUTRAL", "score": 50.0, "label": "🟡 محايد"}
    return ranked, regime
...
def _summary(ranked: pd.DataFrame) -> dict[str, Any]:
    ...
    score = pd.to_numeric(ranked.get("final_opportunity_score", ranked.get("enhanced_opportunity_score", ranked.get("opportunity_score", 0))), errors="coerce").fillna(0)
